...
app.post('/registrarVenta',[verificarToken],async(req,res)=>{
    ...
    try {
        ...
        if(!dataEnvio.idZona || dataEnvio.tipo=='') return res.status(500).json({ok:false,error:'Datos de envio insuficientes.'});
        ...
        return pvService.create(dataVenta.productos,idUltimaVenta).then(()=>{
            res.status(200).json({
                ok:true,
                info:'Venta agregada'
            });
